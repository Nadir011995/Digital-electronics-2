...
ISR(ADC_vect)
{
    ...
    for(i=0; value>0; i++)
    {
        ...
        reminder[i]=value%2; // store the 1큦 and 0큦
        value=value/2;   
    ...
    for (j=i-1 ;j>0; j--)
    {
        if (reminder[j] == 1) 
        {
            count_ones++;  // we want to count only the 1큦
        }
         
    } 
     
    lcd_gotoxy(15,1);
    if ((count_ones%2) == 1) // if the result of this is equal 1 then it has an ODD number of 1큦
    {
